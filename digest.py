#!/usr/bin/env python3
"""
Makanapa daily digest — "what's new & good" as a file you can read or email.
...
def to_markdown(authority, fresh, region, city, days) -> str:
    ...
    L = [f"# 🍜 Makanapa — what's new & good in {where}", f"_{today}_", ""]
    if authority:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Generate the Makanapa daily digest.")
    ...
    if args.email:
        ...
        subject = f"🍜 Makanapa — what's new & good ({where})"
